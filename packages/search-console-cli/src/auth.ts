...
// Scopes required for Google Search Console API (read-only)
...
export async function authenticate(): Promise<OAuth2Client> {
  ...
  if (!credentials) {
    ...
    console.log(
      "  1. Environment variables: GOOGLE_SEARCH_CONSOLE_CLIENT_ID and GOOGLE_SEARCH_CONSOLE_CLIENT_SECRET"
    );
    ...
    console.log("  3. Create an OAuth 2.0 Client ID (Desktop app)");
    console.log("  4. Download the JSON file as client_secrets.json");
    process.exit(1);
  }
...
export async function getAuthenticatedClient(): Promise<OAuth2Client> {
  const credentials = getCredentials();
  if (!credentials) {
    error('No credentials found. Run "gsc auth" first.');
    process.exit(1);
  }
  ...
  if (!existingToken) {
    error('Not authenticated. Run "gsc auth" first.');
    process.exit(1);
  }
  ...
  if (Date.now() >= expiryDate - 60000 && existingToken.refresh_token) {
    try {
      ...
      error('Token expired and refresh failed. Run "gsc auth" again.');
      process.exit(1);
    }
  }
