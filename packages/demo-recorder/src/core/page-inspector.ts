 ...
 */
function generateSelectors(element: Element, index: number): SelectorStrategies {
  const el = element as HTMLElement;
  const selectors: SelectorStrategies = {
  ...
  if (role && accessibleName) {
    selectors.role = `${role}:has-text("${accessibleName.substring(0, 30)}")`;
  }

  // Text selector for buttons and links
...
function generateCssSelector(el: HTMLElement): string {
  ...
  if (el.className && typeof el.className === 'string') {
    ...
    if (classes.length > 0) {
      ...
      // Check if unique (we can't do this in browser context, so we'll use this as fallback)
      return selector;
    }
  }
  ...
  // Fall back to tag + nth-of-type (requires document context)
